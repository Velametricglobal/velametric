...
export const SectionRenderer: React.FC<SectionRendererProps> = ({ section }) => {
  ...
  // 4B. FASHION & MODEL PORTFOLIO PHOTOSHOOT SHOWCASE
  if (section.section_type === 'fashion_gallery' || section.id.includes('fashion') || section.id.includes('model-shoot')) {
    ...
    return (
      <section className="py-16 sm:py-28 bg-zinc-950 border-b border-zinc-800/80">
        <div className="max-w-[1320px] mx-auto px-4 sm:px-6 space-y-10 sm:space-y-12">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 sm:gap-6">
            <div>
              <span className="text-xs font-bold uppercase tracking-widest text-amber-400 block mb-2 font-mono flex items-center gap-1.5">
                <Camera className="w-3.5 h-3.5" /> {content.tagline || 'Velametric Fashion & Media Studio'}
              </span>
              <h2 className="text-2xl sm:text-5xl font-black text-white font-display uppercase tracking-tight">
                {content.heading || 'Model Portfolio & Fashion Shoots.'}
              </h2>
              <p className="text-zinc-400 text-xs sm:text-base mt-2 max-w-2xl">
                {content.subheading || 'High-fashion model portfolios, commercial lookbooks, and botanical couture photography captured in natural Himalayan daylight.'}
              </p>
            </div>
            <SmartLink
              to="/portfolio"
              className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-white text-black font-extrabold text-xs uppercase tracking-wider hover:bg-zinc-200 transition-all shadow-xl touch-target"
            >
              Explore Full Shoot Gallery <ArrowRight className="w-4 h-4" />
            </SmartLink>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
            {shootPhotos.map((photo, idx) => (
              <div
                key={idx}
                className="group relative rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900 h-64 sm:h-80 shadow-xl transition-all hover:border-amber-500/50"
              >
                <img
                  src={photo.url}
                  alt={photo.title}
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3.5">
                  <span className="text-[9px] font-mono text-amber-400 font-bold uppercase">{photo.tag}</span>
                  <span className="text-xs font-bold text-white font-display">{photo.title}</span>
                </div>
              </div>
            ))}
          </div>
