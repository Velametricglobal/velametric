  ...
  return (
    <div className="space-y-8 text-xs text-slate-700 dark:text-slate-200">
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-bold text-slate-900 dark:text-white font-display">1. Select Role to View / Configure Matrix</h2>
          ...
          {SYSTEM_ROLES_CONFIG.map((r) => (
            <button
              ...
              className={`p-3.5 rounded-2xl border text-left transition-all ${
                selectedRole === r.role
                  ? 'bg-brand-50 dark:bg-brand-600/20 border-brand-500 text-brand-900 dark:text-white shadow-sm'
            ...
            >
              <div className="font-bold text-slate-900 dark:text-white text-xs">{r.title}</div>
              <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono truncate mt-0.5">{r.role}</div>
            </button>
          ))}
            ...
            <button
              ...
              className={`p-3.5 rounded-2xl border text-left transition-all ${
                selectedRole === cr.name
                  ? 'bg-amber-50 dark:bg-amber-500/20 border-amber-500 text-amber-900 dark:text-white shadow-sm'
                  : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              <div className="font-bold text-amber-700 dark:text-amber-400 text-xs">{cr.name}</div>
              <div className="text-[10px] text-amber-600/70 dark:text-amber-300/60 font-mono">CUSTOM ROLE</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-6 shadow-sm">
        <div className="flex justify-between items-center">
          <div>
            <span className="text-[10px] font-mono text-amber-600 dark:text-amber-400 uppercase font-bold">Active Scope</span>
          ...
          <div className="inline-flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
            <Info className="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" /> Backend RLS Enforced
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead className="bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 uppercase font-mono text-[10px]">
              <tr>
                <th className="p-4 border-b border-slate-200 dark:border-slate-800">Module / Resource</th>
                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-center">View</th>
                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-center">Create</th>
                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-center">Edit</th>
                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-center">Delete</th>
                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-center">Approve / Launch</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {MODULES_LIST.map((mod) => (
                <tr key={mod.key} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td className="p-4 font-bold text-slate-900 dark:text-white font-display">
                    {mod.name}
                  </td>
                  {['view', 'create', 'edit', 'delete', 'approve'].map((act) => {
                    ...
                    if (!isSupported) {
                      return <td key={act} className="p-4 text-center text-slate-400 dark:text-slate-600 font-mono text-[10px]">N/A</td>;
                    }
                      ...
                      <td key={act} className="p-4 text-center">
                        <button
                          ...
                          className={`w-7 h-7 rounded-lg inline-flex items-center justify-center transition-all ${
                            isAllowed
                              ? 'bg-emerald-500/10 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 border border-emerald-500/30 shadow-sm'
                              : 'bg-slate-100 dark:bg-slate-950 text-slate-400 dark:text-slate-600 border border-slate-200 dark:border-slate-800 hover:text-slate-700 dark:hover:text-slate-400'
                          }`}
                        >
      ...
      {showCreateRoleModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleCreateCustomRole} className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-2xl">
            <h3 className="text-base font-bold text-slate-900 dark:text-white font-display">Create Custom Role</h3>
            ...
            <div>
              <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Role Description</label>
              <textarea
                rows={3}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                className="px-5 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-bold shadow-md transition-all"
              >
                Save Role & Matrix
              </button>
            </div>
