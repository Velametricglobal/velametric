    ...
    >
      ...
      {filter === 'PHOTOSHOOT' && photoshootViewMode === '3D' && photoshootProjects.length > 0 ? (
        <div className="space-y-10 animate-fadeIn">
          ...
          {/* Active 3D Spotlight Lookbook Panel */}
          {current3DProject && (
            <div className="p-6 sm:p-8 rounded-3xl bg-zinc-900/90 border border-zinc-800 text-white space-y-6 shadow-2xl backdrop-blur-xl">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-800 pb-6">
                <div>
                  <span className="text-[10px] font-mono font-bold text-amber-400 uppercase tracking-widest block">
                    {current3DProject.industry} • {current3DProject.completion_date}
                  </span>
                  <h2 className="text-2xl sm:text-3xl font-black font-display uppercase tracking-tight text-white mt-1">
                    {current3DProject.title}
                  </h2>
                  <p className="text-xs text-zinc-400 mt-1">
                    Client: <span className="text-zinc-200 font-bold">{current3DProject.client}</span>
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() =>
                      setActivePhotoModal({
                        images: current3DProject.gallery && current3DProject.gallery.length > 0 ? current3DProject.gallery : [current3DProject.featured_image],
                        index: 0,
                        title: current3DProject.title,
                        client: current3DProject.client,
                        category: current3DProject.industry
                      })
                    }
                    className="px-5 py-2.5 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-extrabold text-xs uppercase tracking-wider transition-all shadow-lg flex items-center gap-2"
                  >
                    <Eye className="w-4 h-4" /> View Full Lookbook ({current3DProject.gallery?.length || 1})
                  </button>

                  <Link
                    to={`/portfolio/${current3DProject.slug}`}
                    className="px-5 py-2.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-white font-bold text-xs uppercase tracking-wider border border-zinc-700 transition-all flex items-center gap-1.5"
                  >
              ...
              {/* 6-Frame Thumbnail Preview Strip */}
              {current3DProject.gallery && current3DProject.gallery.length > 0 && (
                <div className="space-y-3">
                  <div className="text-[11px] font-mono font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2">
                    <Images className="w-3.5 h-3.5 text-amber-400" /> High-Resolution Frame Reel
                  </div>

                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {current3DProject.gallery.slice(0, 6).map((imgUrl, idx) => (
                      <div
                        key={idx}
                        onClick={() =>
                          setActivePhotoModal({
                            images: current3DProject.gallery || [],
                            index: idx,
                            title: current3DProject.title,
                            client: current3DProject.client,
                            category: current3DProject.industry
